...
shared memory 是 GPU 上 block 内 thread 可共享的高速临时内存。它通常比 global memory 快很多，但容量有限。
...
它适合缓存会被同一个 block 内多个 thread 重复使用的数据。它不是更快的 global memory，而是 block 内 thread 协作复用数据的高速工作区。
...
```text
tile 存放在当前 block 的 shared memory 中
同一个 block 内的所有 thread 可以访问它
生命周期通常到当前 block 执行结束为止
```

### __syncthreads

`__syncthreads()` 是 block 内 thread 的同步屏障。

它保证：

```text
同一个 block 内所有 thread 都执行到这里之后，大家才会继续往后执行。
```
...
如果没有同步，有些 thread 可能还没写完 shared memory，其他 thread 就已经开始读，导致读到旧值或未初始化值。

`__syncthreads()` 只能同步同一个 block 内的 thread，不能同步不同 block。

### SM / Streaming Multiprocessor

SM 是 Streaming Multiprocessor，中文常叫流式多处理器。

可以先理解成：

```text
GPU 里面真正执行 thread block 的计算单元集群
```
...
block 会被调度到某个 SM 上执行。每个 SM 里有执行资源，例如 CUDA cores、registers、shared memory 和 warp scheduler。

每个 SM 的 shared memory 总量有限。如果一个 block 使用很多 shared memory，同一个 SM 上能同时驻留的 block 数量通常会减少。

### Occupancy

occupancy 可以粗略理解为一个 SM 上实际驻留的活跃 warps / blocks 相对于硬件上限的比例。

直觉上：

```text
occupancy 越高，GPU 通常越有机会用其他 warp 隐藏 memory latency。
```

但 occupancy 不是越高越一定快。shared memory、register 使用量、block size 都可能影响 occupancy。优化时需要在资源占用和实际性能之间做权衡。
